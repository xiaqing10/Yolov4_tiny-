...
static string labels[3] = {"p1","p2","p3"};
static int GRID0=19;
static int GRID1=38;
static int nclasses=3;
...
void outputs_transform(rknn_output rknn_outputs[], int net_width, int net_height, detection* dets){
    ...
    int masks_0[3] = {3,4,5};
    int masks_1[3] = {1,2,3};
	float anchors[12] = {29, 28,  18, 48,  22,101, 92, 58,  54,134, 134,190};
	//输出xywh均在0-1范围内
...
int draw_image(cv::Mat img,detection* dets,int total,float thresh)
{
	...
	for(int i=0;i<total;i++){
		...
		if(class_>=0) {
			...
			// std::cout << labels[topclass] << "\t@ (" << x1 << ", " << y1 << ") (" << x2 << ", " << y2 << ")" <<  img.cols << img.rows << "\n";

            rectangle(img, Point(x1, y1), Point(x2, y2), Scalar(0, 255, 0, 0), 3);
...
{
	const char *model_path = "yolov4_608.rknn";
	...
	while (true)
	{
		...
		cout << "number boxes left:  " << nboxes_left << endl;
...
void videoRead(const char *video_name) 
{
	...
	{  
		...
		if (queueInput.size() < 30) {
			...
			// 	continue;
			// }
			Mat new_img = cv::imread("608x608.jpg");
			std::cout << new_img.size() << std::endl;
			mtxQueueInput.lock();
			queueInput.push(make_pair(idxInputImage++, new_img));
			mtxQueueInput.unlock();
